	...
	{foreach name=rowIteration from=$data key=id item=rowData}
		...
		{/if}
                ...
                {if $rowData.STATUS == "success" }
                    {assign var='_rowColorStatus' value='style="background-color:#C2FFBA;"'}
                {elseif $rowData.STATUS == "fail" }
                    {assign var='_rowColorStatus' value='style="background-color:#FFC4C4;"'}
                {else}
                    {assign var='_rowColorStatus' value=''}
                {/if}
                    
		<tr height='20' class='{$_rowColor}S1' {$_rowColorStatus}>
			{if $prerow}
